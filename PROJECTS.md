...
- Viewable via native video Picture-in-Picture (canvas → `captureStream()` → PiP), floating on top on both desktop and iOS Safari - confirmed working on both.
...
- **Next step:** none pressing - watch for any remaining Mac-side hook gaps as they come up. Known limitation: stale sessions from force-closed terminals (no graceful `SessionEnd`) only clear after a 30min timeout or a manual `systemctl --user restart clawlight-server.service` - user explicitly chose to leave the timeout as-is rather than shorten it.

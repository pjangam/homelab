...
- **clawlight wired in** (`clawlight/server.py`): edge-triggered on the aggregate entering `waiting`, never per-poll, with a 60s cooldown so a run of permission prompts is one interruption rather than several. Publishes over loopback, so no TLS dependency and the token stays on the host. Respects the existing background-task rule - a session at `waiting` with a subagent still running does **not** alert. Covered by `scripts/test_clawlight_notify.py` (10 cases: flood, miss, cooldown, background-task, multi-session, disabled-when-unconfigured).

...
- **State:** done and running - `clawlight-server.service` and `clawlight-focus-agent.service` live on xero, Caddy proxying `/clawlight`, all 10 hooks wired on xero; Mac hooks being finalized by the user. **Mac still needs:** a `git pull` (for the tmux-reporting `set-status.sh`) and the launchd focus agent from `clawlight/launchd/` - until then Mac sessions show as unreachable.

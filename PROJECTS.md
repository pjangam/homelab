...
- Monitoring closed 2026-09-11: `scripts/check_miraie_ac_available.sh` + a 30min threshold in `cron/healthcheck.sh` now alert by email/ntfy, with a Stats dashboard tile. Both outages that evening were invisible to every existing signal, which is why this watches the HA entity rather than the bridge.
**Next step:** if this recurs often enough to be worth automating, the hook is a notification on exit 2 ("the AC is off the cloud, go switch it on") rather than any restart loop. The retain=false availability is the deeper fix - it would need a change in the ha-miraie-ac node or a Node-RED flow that republishes availability on a timer.
...
**Installing it on another Mac:** `git clone git@github.com:pjangam/homelab.git && homelab/scripts/setup-tmux-shell.sh`. For a machine that shouldn't have the repo, `scripts/setup-tmux-shell.sh --bundle /tmp/install-tmux-shell.sh` emits **one self-contained file** with the three dotfiles appended to it as a payload (read back past an `exit 0`, so bash never parses them) - scp that over and run it. The bundle is generated from `dotfiles/`, never hand-maintained, so it can't drift from the repo copy.
**Notes worth keeping:**
...
**Verified:** `scripts/test_setup_tmux_shell.sh` - 63 checks against throwaway `$HOME` files (fresh machine, migration, foreign-installer takeover, idempotent re-run, `--dry-run`, refusing a `~/.zshrc` that wouldn't parse, and a bundle installing with no repo anywhere near it). Applied to the MacBook and confirmed stable across repeated runs.
...
- **`ntfy` container on xero** (`binwiederhier/ntfy` 2.28.0), config in `ntfy/server.yml`. Bound to `127.0.0.1:8127` only - it is not on the LAN at all, so basic-auth credentials never cross it in plaintext.
- **Its own tailnet hostname, `ntfy.<tailnet>`, via an `ntfy-tailscale` sidecar** - the same pattern HA already used, now factored into a shared `x-tailscale-sidecar` anchor in `docker-compose.yml` plus `ts-serve-config.template.json` + `scripts/render_ts_serve_config.sh`. The template was verified to reproduce HA's existing `serve-config.json` byte-for-byte before adoption, so HA was a no-op.
...
- **Jump to console (added 2026-09-09), borrowed from `clawlight-cli`'s jump-to-terminal.** Clicking a session on the page switches that machine's terminal to the tmux pane it's running in, closing the "shows *which* console needs you, but you still have to go find it" gap. `set-status.sh` now reports `$TMUX`'s socket and `$TMUX_PANE`; a session started outside tmux is shown as **unreachable** rather than offering a jump that would quietly do nothing.
  ...
  - Covered by `scripts/test_clawlight_focus.py` (27 checks: routing, staleness, malformed/injected coordinates) and `scripts/test_clawlight_focus_e2e.sh`, which drives a throwaway tmux server with a real attached client against the live server. Verified on xero against a real live session.
...
- **The Mac's copies are scp'd, not a clone** (`/Users/pramod/code/personal/`), so any change to `set-status.sh` or `focus-agent.sh` needs `clawlight/setup-mac-focus-agent.sh` re-run there - it re-copies both and regenerates the plist. Three separate debugging rounds went on stale Mac copies before this was written down.

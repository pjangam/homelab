...
**Next step:** decide whether to do the GoPro proof-of-concept first (free, just needs the focus/resolution question answered) or go straight to buying an ESP32-CAM (~₹550-950). Either way, still need to build the light-blocking hood/mount and calibrate the 4 pixel coordinates.

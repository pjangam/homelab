...
**Next step:** only remaining item is physical - user to wire the two push buttons to GPIO27/pin 13 and GPIO22/pin 15 (both to a GND pin, e.g. pin 14), no resistor needed. Everything else (script, service, HA automations) is already live and waiting. Separately, still open: approach 2 (tablet), and whether to move to ESP32/ESPHome for locations not near an existing host.

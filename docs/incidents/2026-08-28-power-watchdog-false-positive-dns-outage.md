...
1. **Carrier-loss/restore email notifications** - `projects/power-watchdog/watchdog_power.sh` now emails on the two transition moments it already logs, so a future false-positive-driven wait/shutdown is visible immediately instead of only discovered after the fact. Explicitly a trial, being evaluated for signal-vs-noise before deciding to keep it long-term.

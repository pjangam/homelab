...
**Fix:** `projects/certs-backup/renew_certs.sh` now reloads through Caddy's own admin API instead of signalling the container:
...
Verified end to end: the reload succeeds, `HasBeenManuallyStopped` stays `False` afterwards, caddy keeps serving (200), and a full `./projects/certs-backup/renew_certs.sh` run completes with `exit=0` logging `reloaded caddy via admin API` and leaves the flag clean.

...
# push buttons (GPIO17 start, GPIO18 stop - see scripts/pi-buttons/white-noise-buttons-mqtt.py),
...
- id: white_noise_button_start
  ...
  description: Momentary GPIO push button on the wol-sender Pi (scripts/pi-buttons/white-noise-buttons-mqtt.py) turns on white noise directly - no persisted state involved, so there's nothing for HA/MQTT to resync or override on reconnect.
  triggers:
  - trigger: mqtt
...
- id: white_noise_button_stop
  ...
  description: Momentary GPIO push button on the wol-sender Pi (scripts/pi-buttons/white-noise-buttons-mqtt.py) turns off white noise directly - no persisted state involved, so there's nothing for HA/MQTT to resync or override on reconnect.
  triggers:
  - trigger: mqtt

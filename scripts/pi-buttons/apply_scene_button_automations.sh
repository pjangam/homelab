...
- id: scene_button_oju_sleep
  ...
  description: Momentary GPIO push button on the wol-sender Pi (scripts/pi-buttons/scene-buttons-mqtt.py) fires the oju sleeping scene directly - no persisted state involved, so there's nothing for HA/MQTT to resync or override on reconnect.
  triggers:
  - trigger: mqtt
...
- id: scene_button_oju_awake
  ...
  description: Momentary GPIO push button on the wol-sender Pi (scripts/pi-buttons/scene-buttons-mqtt.py) fires the oju awake scene directly - no persisted state involved, so there's nothing for HA/MQTT to resync or override on reconnect.
  triggers:
  - trigger: mqtt

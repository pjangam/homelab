...
- Token lifetime: the node logs in once at startup and the refresh reuses that token. A MirAIe login returns `expiresIn` = 7257599s, which is **84 days** (measured 2026-09-15 with `projects/miraie-ac/check_miraie_token_lifetime.sh`, which does a separate login and prints no secrets; the node's refresh kept working afterwards). Every node-red restart logs in again and resets the 84 days, and the watchdog, the fix script and image updates all restart it, so an expiry would need node-red to run for 12 weeks straight. If that ever happens, the refresh fails quietly: nothing is published, and the node logs `There was an error getting device status` (`ssh pramod@192.168.1.124 'docker logs --since 24h node-red | grep -i error'`).

...
- Not yet known: whether the MirAIe access token the node gets at startup expires. If it does, the refresh fails quietly: nothing is published, and the node logs `There was an error getting device status` (`ssh pramod@192.168.1.124 'docker logs --since 24h node-red | grep -i error'`). That does no harm, but it stops preventing the problem until the next restart.

...
- The `ha` and `ntfy` hostnames are **immune** to all of this: their tailscale sidecars provision and renew certs internally, with no cron, no sudo and no files in `certs/`.
- **Decided 2026-09-05: Vaultwarden and projects-ui stay under `xero.<tailnet>`.** Moving them to their own sidecar hostnames would retire `renew_certs.sh` and `check_certs.sh` entirely, but it is not worth the cost: Vaultwarden's `DOMAIN` is pinned to `https://xero.<tailnet>`, so a new hostname would invalidate passkey/WebAuthn origins and force every client to be re-pointed, and projects-ui and clawlight are path-routed under the same name. The renewal + independent-check machinery is therefore **permanent, not a stopgap** — maintain it accordingly rather than treating it as something about to be replaced.
